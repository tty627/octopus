...
加密 Office 文件、加密/损坏 ZIP、异常压缩方法或超过资源预算的内容可能只显示文件名和元数据。查看资料页中的解析警告；这类文件不会被误报为已完成正文解析。

PDF 或文本显示“识别质量低”时，低质量正文不会参与排名，以避免乱码污染结果。可以按文件名查找或重新处理。

ZIP 成员无法打开时，先确认外层压缩包仍可访问、成员未改名且内容哈希未变化。来源变化会把相关资料包项目标记为待重新核验，不会自动串接到同名但不同内容的成员。
